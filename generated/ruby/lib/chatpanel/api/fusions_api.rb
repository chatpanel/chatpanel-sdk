...
module ChatPanel
  class FusionsApi
    ...
    # @return [FusionList]
    def fusions_list(opts = {})
      data, _status_code, _headers = fusions_list_with_http_info(opts)
      data
    end
    ...
    # Derived from state (the entity detector&#39;s union once a companion is ready, an engine drafting with a second model) and composed by the user (&#x60;POST /config { fusions }&#x60;, a chat fallback in order). A chat turn names a fallback as &#x60;model: \&quot;fusion:&lt;id&gt;\&quot;&#x60; and is routed to the first member that is up; &#x60;x-chatpanel-fusion&#x60; on the response says which. 
    # @param [Hash] opts the optional parameters
    # @return [Array<(FusionList, Integer, Hash)>] FusionList data, response status code and response headers
    def fusions_list_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: FusionsApi.fusions_list ...'
      end
      # resource path
      local_var_path = '/v1/fusions'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'FusionList'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['gatewayToken']

      new_options = opts.merge(
        :operation => :"FusionsApi.fusions_list",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: FusionsApi#fusions_list\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
