...
module ChatPanel
  class SmartSearchRequest < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::SmartSearchRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::SmartSearchRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'max_queries')
        self.max_queries = attributes[:'max_queries']
      else
        self.max_queries = 4
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@max_queries.nil? && @max_queries > 6
        invalid_properties.push('invalid value for "max_queries", must be smaller than or equal to 6.')
      end

      if !@max_queries.nil? && @max_queries < 1
        invalid_properties.push('invalid value for "max_queries", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@max_queries.nil? && @max_queries > 6
      return false if !@max_queries.nil? && @max_queries < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] max_queries Value to be assigned
    def max_queries=(max_queries)
      if max_queries.nil?
        fail ArgumentError, 'max_queries cannot be nil'
      end

      if max_queries > 6
        fail ArgumentError, 'invalid value for "max_queries", must be smaller than or equal to 6.'
      end

      if max_queries < 1
        fail ArgumentError, 'invalid value for "max_queries", must be greater than or equal to 1.'
      end
    ...
    def hash
      [type, since, before, limit, question, queries, max_queries].hash
    end
