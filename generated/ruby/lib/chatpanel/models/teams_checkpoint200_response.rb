...
module ChatPanel
  class TeamsCheckpoint200Response < ApiModelBase
    attr_accessor :ok
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::TeamsCheckpoint200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::TeamsCheckpoint200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'checkpoint')
        if (value = attributes[:'checkpoint']).is_a?(Hash)
    ...
    def list_invalid_properties
      ...
      if @checkpoint.nil?
        invalid_properties.push('invalid value for "checkpoint", checkpoint cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @checkpoint.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] checkpoint Value to be assigned
    def checkpoint=(checkpoint)
      if checkpoint.nil?
        fail ArgumentError, 'checkpoint cannot be nil'
      end
