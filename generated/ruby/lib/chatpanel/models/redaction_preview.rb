...
module ChatPanel
  class RedactionPreview < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::RedactionPreview` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::RedactionPreview`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sanitized')
        self.sanitized = attributes[:'sanitized']
      end
      ...
      if attributes.key?(:'detector')
        if (value = attributes[:'detector']).is_a?(Hash)
          self.detector = value
        end
      end
    end
    ...
    def hash
      [text, count, sanitized, tier, entities, detector].hash
    end
