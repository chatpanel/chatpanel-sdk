...
module ChatPanel
  class RuntimeDocumentProcesses < ApiModelBase
    ...
    def self.openapi_types
      {
        :'all' => :'Array<Hash>',
        :'local_mcp' => :'Array<Hash>',
        :'warm' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::RuntimeDocumentProcesses` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::RuntimeDocumentProcesses`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'local_mcp')
        if (value = attributes[:'local_mcp']).is_a?(Array)
          self.local_mcp = value
        end
      end

      if attributes.key?(:'warm')
        if (value = attributes[:'warm']).is_a?(Hash)
          self.warm = value
        end
      end
    end
