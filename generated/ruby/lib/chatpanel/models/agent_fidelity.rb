...
module ChatPanel
  ...
  class AgentFidelity < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'dropped' => :'Array<AgentFidelityDroppedInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::AgentFidelity` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::AgentFidelity`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dropped')
        if (value = attributes[:'dropped']).is_a?(Array)
          self.dropped = value
        end
      end
    end
    ...
    def hash
      [dialect, lossless, carried, dropped].hash
    end
