...
module ChatPanel
  class ProjectsRecruitRequest < ApiModelBase
    attr_accessor :by

    attr_accessor :reach

    attr_accessor :chat_model
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::ProjectsRecruitRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::ProjectsRecruitRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'chat_model')
        self.chat_model = attributes[:'chat_model']
      end

      if attributes.key?(:'evaluation')
        if (value = attributes[:'evaluation']).is_a?(Hash)
    ...
    def hash
      [by, reach, chat_model, evaluation, text].hash
    end
