...
module ChatPanel
  class DecideQuestion < ApiModelBase
    attr_accessor :type
    ...
    def self.openapi_types
      {
        :'type' => :'String',
        :'instructions' => :'String',
        :'options' => :'Array<DecideQuestionOptionsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::DecideQuestion` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::DecideQuestion`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @instructions.to_s.length < 1
        invalid_properties.push('invalid value for "instructions", the character length must be greater than or equal to 1.')
      end

      if !@options.nil? && @options.length > 255
        invalid_properties.push('invalid value for "options", number of items must be less than or equal to 255.')
      end

      if !@options.nil? && @options.length < 2
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["choice", "score", "noul"])
      return false unless type_validator.valid?(@type)
      return false if @instructions.nil?
      return false if @instructions.to_s.length < 1
      return false if !@options.nil? && @options.length > 255
      return false if !@options.nil? && @options.length < 2
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["choice", "score", "noul"])
    ...
    def instructions=(instructions)
      ...
      if instructions.to_s.length < 1
        fail ArgumentError, 'invalid value for "instructions", the character length must be greater than or equal to 1.'
      end
    ...
    def options=(options)
      ...
      if options.length > 255
        fail ArgumentError, 'invalid value for "options", number of items must be less than or equal to 255.'
      end
    ...
    def hash
      [type, instructions, options].hash
    end
