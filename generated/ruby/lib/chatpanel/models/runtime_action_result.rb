...
module ChatPanel
  class RuntimeActionResult < ApiModelBase
    attr_accessor :ok
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::RuntimeActionResult` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::RuntimeActionResult`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'already')
        self.already = attributes[:'already']
      end
      ...
      if attributes.key?(:'answering')
        self.answering = attributes[:'answering']
      end

      if attributes.key?(:'install')
        if (value = attributes[:'install']).is_a?(Hash)
          self.install = value
        end
      end
      ...
      if attributes.key?(:'restarted')
        self.restarted = attributes[:'restarted']
      end
    ...
    def hash
      [ok, error, already, url, answering, install, provides, model, restarted, note].hash
    end
