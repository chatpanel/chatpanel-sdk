...
module ChatPanel
  class DecideResponse < ApiModelBase
    ...
    def self.openapi_types
      {
        :'answers' => :'Hash<String, DecideAnswer>',
        :'model' => :'String',
        :'ms' => :'Float',
        :'calibrated' => :'Boolean'
      }
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::DecideResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::DecideResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'calibrated')
        self.calibrated = attributes[:'calibrated']
      end
    end
    ...
    def hash
      [answers, model, ms, calibrated].hash
    end
