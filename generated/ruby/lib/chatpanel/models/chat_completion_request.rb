...
module ChatPanel
  class ChatCompletionRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'stream' => :'Boolean',
        :'stream_options' => :'Hash<String, Object>',
        :'tools' => :'Array<Hash>',
        :'tool_choice' => :'Object',
        :'temperature' => :'Float',
        :'max_tokens' => :'Integer'
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::ChatCompletionRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::ChatCompletionRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'stream_options')
        if (value = attributes[:'stream_options']).is_a?(Hash)
    ...
    def hash
      [model, messages, stream, stream_options, tools, tool_choice, temperature, max_tokens].hash
    end
