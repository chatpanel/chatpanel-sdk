...
module ChatPanel
  class PushEventsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::PushEventsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::PushEventsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @events.length > 2000
        invalid_properties.push('invalid value for "events", number of items must be less than or equal to 2000.')
      end
    ...
    def valid?
      ...
      return false if @events.length > 2000
      true
    end
    ...
    def events=(events)
      ...
      if events.length > 2000
        fail ArgumentError, 'invalid value for "events", number of items must be less than or equal to 2000.'
      end
