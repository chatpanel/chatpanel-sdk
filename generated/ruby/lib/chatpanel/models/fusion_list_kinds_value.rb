...
module ChatPanel
  class FusionListKindsValue < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::FusionListKindsValue` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::FusionListKindsValue`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'what')
        self.what = attributes[:'what']
      end
    ...
    def valid?
      ...
      gain_validator = EnumAttributeValidator.new('String', ["recall", "speed", "reliability"])
      return false unless gain_validator.valid?(@gain)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] gain Object to be assigned
    def gain=(gain)
      validator = EnumAttributeValidator.new('String', ["recall", "speed", "reliability"])
      unless validator.valid?(gain)
        fail ArgumentError, "invalid value for \"gain\", must be one of #{validator.allowable_values}."
      end
      @gain = gain
    end
