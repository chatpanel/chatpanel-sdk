...
module ChatPanel
  class ReadSection < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::ReadSection` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::ReadSection`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @heading.nil?
        invalid_properties.push('invalid value for "heading", heading cannot be nil.')
      end
      ...
      if @level > 6
        invalid_properties.push('invalid value for "level", must be smaller than or equal to 6.')
      end

      if @level < 1
        invalid_properties.push('invalid value for "level", must be greater than or equal to 1.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] heading Value to be assigned
    def heading=(heading)
      if heading.nil?
        fail ArgumentError, 'heading cannot be nil'
      end
    ...
    def level=(level)
      ...
      if level > 6
        fail ArgumentError, 'invalid value for "level", must be smaller than or equal to 6.'
      end
