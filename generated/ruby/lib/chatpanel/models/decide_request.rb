...
module ChatPanel
  class DecideRequest < ApiModelBase
    # The text judged.
    attr_accessor :state
    ...
    def self.openapi_types
      {
        :'state' => :'String',
        :'questions' => :'Hash<String, DecideQuestion>',
        :'model' => :'String',
        :'budget_ms' => :'Float'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::DecideRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::DecideRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'questions')
        if (value = attributes[:'questions']).is_a?(Hash)
    ...
    def hash
      [state, questions, model, budget_ms].hash
    end
