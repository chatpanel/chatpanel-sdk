...
module ChatPanel
  class PutRecordsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::PutRecordsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::PutRecordsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@host.nil? && @host.to_s.length > 80
        invalid_properties.push('invalid value for "host", the character length must be smaller than or equal to 80.')
      end
    ...
    def valid?
      ...
      return false if !@host.nil? && @host.to_s.length > 80
      true
    end
    ...
    def host=(host)
      ...
      if host.to_s.length > 80
        fail ArgumentError, 'invalid value for "host", the character length must be smaller than or equal to 80.'
      end
