...
module ChatPanel
  class EventsStats < ApiModelBase
    attr_accessor :ok
    ...
    def self.openapi_types
      {
        :'ok' => :'Boolean',
        :'stats' => :'EventsStatsStats',
        :'year' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::EventsStats` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::EventsStats`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'year')
        if (value = attributes[:'year']).is_a?(Hash)
          self.year = value
        end
      end
    end
