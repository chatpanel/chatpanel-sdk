...
module ChatPanel
  class DecideAnswer < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::DecideAnswer` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::DecideAnswer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @p.nil?
        invalid_properties.push('invalid value for "p", p cannot be nil.')
      end

      if @p > 1
        invalid_properties.push('invalid value for "p", must be smaller than or equal to 1.')
      end
    ...
    def p=(p)
      if p.nil?
        fail ArgumentError, 'p cannot be nil'
      end

      if p > 1
        fail ArgumentError, 'invalid value for "p", must be smaller than or equal to 1.'
      end
    ...
    def hash
      [value, p, options, confidence].hash
    end
