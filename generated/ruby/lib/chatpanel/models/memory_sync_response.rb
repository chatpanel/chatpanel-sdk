...
module ChatPanel
  class MemorySyncResponse < ApiModelBase
    attr_accessor :ok
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::MemorySyncResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::MemorySyncResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @memories.nil?
        invalid_properties.push('invalid value for "memories", memories cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] memories Value to be assigned
    def memories=(memories)
      if memories.nil?
        fail ArgumentError, 'memories cannot be nil'
      end
