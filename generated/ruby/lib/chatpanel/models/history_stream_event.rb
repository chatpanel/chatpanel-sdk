...
module ChatPanel
  class HistoryStreamEvent < ApiModelBase
    # The SSE event name.
    attr_accessor :event
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::HistoryStreamEvent` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::HistoryStreamEvent`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      event_validator = EnumAttributeValidator.new('String', ["hello", "records"])
    ...
    def event=(event)
      validator = EnumAttributeValidator.new('String', ["hello", "records"])
    ...
    def hash
      [event, newest, size, version, ids, at].hash
    end
