...
module ChatPanel
  class SkillsQuarantined200Response < ApiModelBase
    ...
    def self.openapi_types
      {
        :'quarantined' => :'Array<QuarantinedSkill>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::SkillsQuarantined200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::SkillsQuarantined200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'quarantined')
        if (value = attributes[:'quarantined']).is_a?(Array)
          self.quarantined = value
        end
      else
        self.quarantined = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @quarantined.nil?
        invalid_properties.push('invalid value for "quarantined", quarantined cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @quarantined.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] quarantined Value to be assigned
    def quarantined=(quarantined)
      if quarantined.nil?
        fail ArgumentError, 'quarantined cannot be nil'
      end
