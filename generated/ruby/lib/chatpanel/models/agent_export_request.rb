...
module ChatPanel
  class AgentExportRequest < ApiModelBase
    attr_accessor :agent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChatPanel::AgentExportRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChatPanel::AgentExportRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      to_validator = EnumAttributeValidator.new('String', ["claude", "codex", "chatpanel"])
    ...
    def to=(to)
      validator = EnumAttributeValidator.new('String', ["claude", "codex", "chatpanel"])
