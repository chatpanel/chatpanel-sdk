...
/// struct for typed errors of method [`fusions_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FusionsListError {
...
pub async fn fusions_list(configuration: &configuration::Configuration, ) -> Result<models::FusionList, Error<FusionsListError>> {

    let uri_str = format!("{}/v1/fusions", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FusionList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FusionList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FusionsListError> = serde_json::from_str(&content).ok();
