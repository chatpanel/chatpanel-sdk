package net.chatpanel.sdk.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val BASE_URL_KEY: String = "net.chatpanel.sdk.baseUrl"
        @Deprecated(
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["X-ChatPanel-Token"].isNullOrEmpty()) {
            if (apiKey["X-ChatPanel-Token"] != null) {
                if (apiKeyPrefix["X-ChatPanel-Token"] != null) {
                    requestConfig.headers["X-ChatPanel-Token"] = apiKeyPrefix["X-ChatPanel-Token"]!! + " " + apiKey["X-ChatPanel-Token"]!!
                } else {
                    requestConfig.headers["X-ChatPanel-Token"] = apiKey["X-ChatPanel-Token"]!!
                }
            }
        }
    }
